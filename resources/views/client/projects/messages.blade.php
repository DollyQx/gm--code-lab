...
<div class="space-y-6 max-w-6xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-700/60 pb-5">
        <div>
            <div class="flex items-center space-x-3">
                ...
                <h1 class="text-2xl font-bold text-white tracking-tight">Project Messages</h1>
            ...
            <p class="text-sm text-slate-400 mt-1 ml-8">Direct project communication with GM Code Lab team</p>
        </div>
        <div class="flex items-center space-x-3">
            <a href="{{ route('client.projects.show', $project->id) }}" class="px-4 py-2 text-sm font-medium bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-lg transition-colors">
                Back to Workspace
            </a>
        </div>
    </div>
    ...
    <div class="bg-slate-800/80 border border-slate-700/60 rounded-xl overflow-hidden shadow-xl flex flex-col min-h-[500px]">
        ...
        <div class="p-6 overflow-y-auto space-y-6 flex-1 max-h-[600px] bg-slate-900/40">
                ...
                <div class="flex flex-col {{ $isCurrentUser ? 'items-end' : 'items-start' }}">
                    <div class="flex items-center space-x-2 mb-1.5 px-1">
                        <span class="text-xs font-semibold {{ $isStaff ? 'text-indigo-400' : 'text-emerald-400' }}">
                            {{ $isStaff ? 'GM Code Lab Team (' . $msg->sender?->name . ')' : $msg->sender?->name }}
                        </span>
                    ...
                    <div class="max-w-xl rounded-2xl p-4 text-sm shadow-md {{ $isCurrentUser ? 'bg-emerald-600 text-white rounded-tr-none' : ($isStaff ? 'bg-slate-700/90 text-slate-100 rounded-tl-none border border-slate-600/50' : 'bg-slate-800 text-slate-100 rounded-tl-none border border-slate-700') }}">
                        @if($msg->message)
                        ...
                        @if($msg->attachment_path)
                            <div class="mt-3 pt-2 border-t border-white/20">
                                <a href="{{ route('client.projects.messages.download', [$project->id, $msg->id]) }}" 
            ...
            @empty
                <div class="text-center py-16">
                    ...
                    <p class="text-slate-500 text-xs mt-1">Send a message to our team below.</p>
                </div>
            @endforelse
        </div>
        ...
        <div class="p-4 bg-slate-800 border-t border-slate-700/60">
            <form method="POST" action="{{ route('client.projects.messages.store', $project->id) }}" enctype="multipart/form-data" class="space-y-3">
                @csrf
                <div>
                    <textarea name="message" rows="3" placeholder="Type your project message here..." 
                        class="w-full bg-slate-900/90 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 resize-none">{{ old('message') }}</textarea>
                </div>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-1">
                    ...
                    <button type="submit" class="inline-flex items-center justify-center space-x-2 px-6 py-2.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold transition-colors shadow-lg">
