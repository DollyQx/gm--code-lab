...
@section('title', 'Edit Quotation ' . $quotation->reference_number)
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Edit Quotation</h1>
                <span class="font-mono text-sm font-bold text-blue-600 bg-blue-50 border border-blue-200 px-2.5 py-1 rounded-lg">
                    {{ $quotation->reference_number }}
                </span>
            </div>
            <p class="text-sm text-slate-500 mt-1">Modify proposal headers, terms, or update line item amounts.</p>
        </div>
        <a href="{{ route('admin.quotations.show', $quotation->id) }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-sm rounded-xl transition-colors">
            Back to View
        </a>
    </div>

    <form action="{{ route('admin.quotations.update', $quotation->id) }}" method="POST" id="quotation-form" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
                ...
                <div>
                    ...
                    <select name="client_id" id="client_id" required class="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ (string) old('client_id', $quotation->client_id) === (string) $client->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="project_id" id="project_id" class="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                        ...
                        @foreach($projects as $project)
                            <option value="{{ $project->id }}" {{ (string) old('project_id', $quotation->project_id) === (string) $project->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="lead_id" id="lead_id" class="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                        ...
                        @foreach($leads as $lead)
                            <option value="{{ $lead->id }}" {{ (string) old('lead_id', $quotation->lead_id) === (string) $lead->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-4 border-t border-slate-100">
                ...
                <div>
                    ...
                    <input type="date" name="issue_date" id="issue_date" value="{{ old('issue_date', $quotation->issue_date ? $quotation->issue_date->format('Y-m-d') : '') }}" required class="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="valid_until" id="valid_until" value="{{ old('valid_until', $quotation->valid_until ? $quotation->valid_until->format('Y-m-d') : '') }}" required class="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" required class="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">
                        @foreach(\App\Enums\QuotationStatus::cases() as $st)
                            <option value="{{ $st->value }}" {{ old('status', $quotation->status->value) === $st->value ? 'selected' : '' }}>
                                {{ $st->label() }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="4" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">{{ old('notes', $quotation->notes) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="terms" id="terms" rows="4" class="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500">{{ old('terms', $quotation->terms) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('admin.quotations.show', $quotation->id) }}" class="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-sm rounded-xl transition-colors">
                Cancel
            </a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initialize with existing items or old input items
        @if(old('items'))
            const initialItems = @json(old('items'));
            Object.values(initialItems).forEach(item => createRow(item));
        @else
            const existingItems = @json($quotation->items);
            if (existingItems && existingItems.length > 0) {
                existingItems.forEach(item => createRow({
