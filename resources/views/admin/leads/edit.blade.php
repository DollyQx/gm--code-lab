...
@section('title', 'Edit Lead: ' . $lead->reference_number)
@section('breadcrumb', 'Leads / Edit ' . $lead->reference_number)
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <a href="{{ route('admin.leads.show', $lead->id) }}" class="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:text-blue-700 mb-2">
                &larr; Back to Lead Details
            </a>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Lead Proposal: <span class="font-mono text-blue-600">{{ $lead->reference_number }}</span></h1>
            <p class="text-sm text-slate-500">Update client lead details, pipeline status, and staff assignment.</p>
        </div>
    </div>

    <!-- Form Container -->
    <form action="{{ route('admin.leads.update', $lead->id) }}" method="POST" class="bg-white p-6 sm:p-8 rounded-xl border border-slate-200 shadow-sm space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $lead->name) }}" required class="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Email Address *</label>
                    <input type="email" name="email" value="{{ old('email', $lead->email) }}" required class="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Phone Number</label>
                    <input type="text" name="phone" value="{{ old('phone', $lead->phone) }}" class="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Company / Organization</label>
                    <input type="text" name="company_name" value="{{ old('company_name', $lead->company_name) }}" class="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="source" value="{{ old('source', $lead->source) }}" class="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>
                ...
                <div>
                    ...
                    <select name="assigned_user_id" class="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                        ...
                        @foreach($staffUsers as $staff)
                            <option value="{{ $staff->id }}" {{ old('assigned_user_id', $lead->assigned_user_id) == $staff->id ? 'selected' : '' }}>
            ...
            <div class="mt-4">
                ...
                <select name="client_id" class="w-full px-3.5 py-2 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    ...
                    @foreach($clients as $c)
                        <option value="{{ $c->id }}" {{ old('client_id', $lead->client_id) == $c->id ? 'selected' : '' }}>
                            {{ $c->name }} — {{ $c->email }}
                        </option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <div>
                ...
                <textarea name="notes" rows="4" class="w-full px-3.5 py-2.5 text-sm rounded-lg border border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">{{ old('notes', $lead->notes) }}</textarea>
            </div>
        </div>

        <!-- Actions -->
        <div class="pt-4 border-t border-slate-200 flex items-center justify-end gap-3">
            <a href="{{ route('admin.leads.show', $lead->id) }}" class="btn-base btn-outline btn-sm">
                Cancel
            </a>
