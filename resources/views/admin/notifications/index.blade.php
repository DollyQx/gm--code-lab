...
<div class="space-y-6 max-w-5xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Notifications Center</h1>
                @if($unreadCount > 0)
            ...
            <p class="text-sm text-slate-500 mt-1">Stay informed on system activity, payment alerts, and client actions.</p>
        </div>

        @if($unreadCount > 0)
            <form action="{{ route('admin.notifications.mark-all-read') }}" method="POST">
                @csrf
                <button type="submit" class="inline-flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold px-4 py-2.5 rounded-xl shadow-sm transition-colors">
    ...
    <div class="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <div class="flex items-center gap-2">
            <a href="{{ route('admin.notifications.index') }}" class="px-3 py-1.5 rounded-lg text-xs font-semibold {{ !request('unread') ? 'bg-blue-50 text-blue-600 font-bold' : 'text-slate-600 hover:bg-slate-100' }}">
                All Notifications
            </a>
            <a href="{{ route('admin.notifications.index', ['unread' => 1]) }}" class="px-3 py-1.5 rounded-lg text-xs font-semibold {{ request('unread') ? 'bg-blue-50 text-blue-600 font-bold' : 'text-slate-600 hover:bg-slate-100' }}">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm divide-y divide-slate-100 overflow-hidden">
        @if($notifications->isEmpty())
            <div class="p-12 text-center">
                ...
                <p class="text-xs text-slate-500 mt-1">You're all caught up! No notifications available right now.</p>
            </div>
        @else
            @foreach($notifications as $notification)
                ...
                <div class="p-5 flex items-start justify-between gap-4 transition-colors {{ $isUnread ? 'bg-blue-50/40 border-l-4 border-l-blue-600' : 'hover:bg-slate-50/60' }}">
                    <div class="flex items-start gap-3.5">
                        ...
                        <div class="space-y-1">
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="text-xs font-bold text-slate-900">{{ $data['title'] ?? 'System Notification' }}</span>
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                        ...
                        @else
                            <form action="{{ route('admin.notifications.mark-unread', $notification->id) }}" method="POST">
                                @csrf
