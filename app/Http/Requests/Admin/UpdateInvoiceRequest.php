...
class UpdateInvoiceRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            if ($invoice && in_array($invoice->status->value, [InvoiceStatus::PAID->value, InvoiceStatus::CANCELLED->value])) {
                // If the invoice is finalized, prevent editing financial totals unless the status is reset
                $newStatus = $this->input('status');
                if ($newStatus === $invoice->status->value) {
                    $validator->errors()->add('status', 'Finalized invoices (Paid or Cancelled) cannot be edited directly.');
                }
            }
