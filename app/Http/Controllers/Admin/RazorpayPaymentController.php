...
class RazorpayPaymentController extends Controller
{
    ...
    /**
     * Create Razorpay Order for an Invoice server-side.
     */
    public function createOrder(Request $request, Invoice $invoice): JsonResponse
    {
        ...
        // Optional custom requested payment amount (e.g. partial payment)
        $requestedAmount = $request->input('amount') ? (float) $request->input('amount') : $amountDue;
        if ($requestedAmount <= 0 || $requestedAmount > $amountDue + 0.01) {
            return response()->json([
                'success' => false,
        ...
        try {
            $order = $this->razorpayService->createOrder($invoice, $requestedAmount);
            ...
            Payment::updateOrCreate(
                ...
                [
                    ...
                    'notes' => 'Razorpay online payment order created.',
                ]
            );
    ...
    /**
     * Verify Razorpay Checkout Payment Signature server-side.
     */
    public function verifyPayment(Request $request, Invoice $invoice): JsonResponse
    {
        ...
        DB::transaction(function () use ($payment, $paymentId, $signature, $invoice) {
            ...
            ActivityLogger::log(
                ...
                subject: $invoice,
                description: 'Recorded online Razorpay payment of ₹' . number_format((float) $payment->amount, 2) . ' (Ref: ' . $payment->reference_number . ')',
                metadata: [
