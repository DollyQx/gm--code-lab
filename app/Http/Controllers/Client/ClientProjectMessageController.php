...
class ClientProjectMessageController extends Controller
{
    /**
     * Display project communication thread for authenticated Client.
     */
    public function index(Request $request, Project $project): View
    {
        ...
        if ((int) $project->client_id !== (int) $request->user()->id) {
            abort(403, 'Unauthorized access to project messages.');
        }
        ...
        // Mark unread messages as read for authenticated client
        $user = $request->user();
        ...
        return view('client.projects.messages', [
            'project' => $project,
    ...
    /**
     * Post a new project message as Client.
     */
    public function store(Request $request, Project $project): RedirectResponse
    {
        ...
        // Notify authorized staff roles (Admin, Super Admin, Project Manager)
        NotificationService::notifyRoles(
            [UserRole::ADMIN, UserRole::SUPER_ADMIN, UserRole::PROJECT_MANAGER],
            'project_message',
            'Client Message on ' . $project->title,
            $request->user()->name . ' sent a message on project ' . $project->reference_number,
            route('admin.projects.messages', $project),
            $project
        );
        ...
        ActivityLogger::log(
            ...
            $project,
            'Client sent a message on project ' . $project->reference_number,
            [
        ...
        if ($attachmentPath) {
            ActivityLogger::log(
                ...
                $project,
                'Client uploaded attachment (' . $originalFilename . ') on project ' . $project->reference_number,
                [
                    'message_id' => $message->id,
        ...
        return redirect()->route('client.projects.messages', $project)
